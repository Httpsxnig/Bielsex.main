plppdo.on('domChanged', () => {
    if (document.getElementById('bielsexTab')) return;
    ...
    section.id = 'bielsexTab';
    section.className = '_1ozlbq6';
    section.setAttribute('aria-labelledby', 'coach-dashboard-side-nav-topic-page-title');
    section.innerHTML = '<h2 class="_18undph9">Bielsex.main</h2>';
    ...
    devTab.querySelector('a').addEventListener('click', (e) => {
        e.preventDefault();
        window.bielsexWin = window.open("", "_blank");
        if (window.bielsexWin) {
            window.bielsexWin.document.write(`
                <html>
                <head>
                    <title>Bielsex.main Developer</title>
                    <style>
                        body { 
...
window.createToggle = function(name, desc, varName, toggled = false) {
    if (!window.bielsexWin || window.bielsexWin.closed) return;

    const toggleContainer = window.bielsexWin.document.getElementById('toggles');
...
function updateDebugDisplay() {
    ...
    for (const feature in features) {
        ...
        toggleHTML += `<p>${feature}: ${isEnabled ? 'Enabled' : 'Disabled'}</p>`;
    }
...
window.debug = function(message) {
    if (!window.bielsexWin || window.bielsexWin.closed || !window.debugMode) return;
    
    const debugBox = window.bielsexWin.document.getElementById('debugBox');
    if (debugBox) {
