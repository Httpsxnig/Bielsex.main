...
function setupMain(){
    ...
    (function () {
        ...
        bielsexDominates = true;
        
        (async () => { 
            while (bielsexDominates) {
...
/* Inject */
if (!/^https?:\/\/([a-z0-9-]+\.)?khanacademy\.org/.test(window.location.href)) { alert("❌ Bielsex.main Failed to Injected!\n\nVocê precisa executar o Bielsex.main no site do Khan Academy! (https://pt.khanacademy.org/)"); window.location.href = "https://pt.khanacademy.org/"; }
...
.then(async () => {    
    sendToast("🪶 Bielsex.main Minimal injetado com sucesso!");
